...
curl -sfS $PROFILE_URL | tar xzf - -C www/profiles
...
if ! [ -e $STACKATO_FILESYSTEM/INSTALLED ]
  then
    ...
    $STACKATO_APP_ROOT/drush/drush -r $HOME/www site-install commerce_kickstart -y --db-url=$DATABASE_URL --account-name=admin --account-pass=passwd --site-name=Stackato --locale=en-US
